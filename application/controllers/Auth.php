...
class Auth extends CI_Controller {
	...
	*/
    ...
    {	
		...
		$this->data['page_title'] = 'eFile - Bulk Scanning';
	...
	function logout() 
	{
		...
		$this->flexi_auth->logout(TRUE);
                ...
                // Set a message to the CI flashdata so that it is available after the page redirect.
                if($this->session->flashdata('msg_Flexi_auth_division')){
                    $this->session->set_flashdata('message', '<p class="status_msg">Account Updated Successfull Please Login Again.</p>');
                }else{
                $this->session->set_flashdata('message', $this->flexi_auth->get_messages());		
                }
		redirect('auth');
