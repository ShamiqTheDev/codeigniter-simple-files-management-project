						...
						<div class="panel-body">
							<div class="form-group">
								<div class="form-validation">
									...
									<div class="col-sm-9">
										<select name="receipt_sent_back_to" id="receipt_sent_back_to" class="form-control" disabled>
										
														<option value="">Select</option>
														<?php
															if($send_user) {
																foreach($send_user as $get_send_user) {
																	$selected = (isset($receiptSendBackId) && $get_send_user['uacc_id'] == $receiptSendBackId) ? "selected='selected'" : "";
																	//$user_id = (isset($receiptSendBackId) && $get_send_user['uacc_id'] == $receiptSendBackId) ? $get_send_user['uacc_id'] : "";
																	$designation = (isset($get_send_user['designationName']) && $get_send_user['uacc_id'] == $receiptSendBackId) ? ' ('.$get_send_user['designationName'].')' : '';
																	echo '<option value="'.$get_send_user['uacc_id'].'"'.$selected.'>'.$get_send_user['upro_first_name'].' '.$get_send_user['upro_last_name'].$designation.'</option>';
																}
															}
														?>
										</select>
										<input type="hidden" name="receipt_sent_to" id="receipt_sent_to" value="<?php echo $receiptSendBackId; ?>" />
										<!--<div class="ui fluid search dropdown selection">
											<input type="hidden" name="receipt_sent_to" id="receipt_sent_to">
											...
											<div class="menu">
												...
												if($send_user) {
													foreach($send_user as $get_send_user) {
														if(isset($receiptSendBackId) && $receiptSendBackId == $get_send_user['uacc_id']){
							...
							<!--<div class="form-group">
								...
								<div class="col-sm-9">
									<div class="ui fluid search dropdown selection multiple">
										<input type="hidden" name="receipt_sent_cc">
										<i class="dropdown icon"></i>
										<div class="default text">Select</div>
										<div class="menu">
											<?php
							...
							<!--<div class="form-group">
								...
								<div class="col-sm-9">
									<div class="input-group">
										<input name="due_date" id="due_date" class="form-control datepicker" type="text">
							...
							<hr>
							<input type="hidden" name="send_id_old" id="send_id_old" value="<?php echo $sent_receipt['sentId']; ?>">
					...
					<div class="gray_box">
						...
						<?php
							if($receipt) {
										...
										<div class="panel-body details-info">
											...
											<div class="form-group">
												<div class="form-validation">
													...
													<div class="col-sm-4 file-detail">
														<?php echo date('d-m-Y', strtotime($get_receipt['letterDate'])); ?>
													</div>
												</div>
												<div class="form-validation">
													<label class="col-sm-2 control-label">
								...
								<input type="hidden" id="receipt_sent_back_id" name="receipt_sent_back_id" value="<?php echo $get_receipt['receiptSentId']; ?>" >
								<input type="hidden" id="send_action_id" name="send_action_id" value="<?php echo $get_receipt['sendActionId']; ?>" >
								<input type="hidden" id="send_priority_id" name="send_priority_id" value="<?php echo $get_receipt['sendPriorityId']; ?>" >
								<input type="hidden" id="due_date" name="due_date" value="<?php echo $get_receipt['dueDate']; ?>" >
								<?php
									$i++;
								}
							}
	...
	var FormValidator = function () {
		...
		var fileUploadingForm = function () {
			...
			$('#fileUploadingForm').validate({
				...
				messages: {
					receipt_sent_to: "Please enter to",
				},
