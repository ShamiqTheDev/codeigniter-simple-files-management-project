                        ...
                        <div class="panel-body">
							<div class="form-group">
								<label class="col-sm-3 control-label" for="project_name">
									Ministry Name <span style="color:red;">*</span>
								</label>
								<div class="col-sm-8">
										...
										<?php
											if($ministry) {
												foreach($ministry as $get_ministry) {
													$selected = ($department && $department['ministryId'] == $get_ministry['ministryId']) ? 'selected="selected"' : '';
													
													echo '<option '.$selected.' value="'.$get_ministry['ministryId'].'">'.$get_ministry['ministryName'].'</option>';
												}
											}
										?>
									</select>
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input type="text" name="department_name" id="department_name" value="<?php echo isset($department['departmentName']) ? $department['departmentName'] : '' ?>" class="form-control">
								</div>
							</div>
						</div>
                        <div class="panel-body">
                            <div class="col-md-2 pull-right">
                                <button id="submit_btn" class="btn btn-yellow btn-block" type="submit">
                                    <?php echo isset($department) ? 'Update' : 'Submit'; ?> <i class="fa fa-arrow-circle-right"></i>
								</button>
							</div>
						</div>
                        <?php echo form_close(); ?>
	...
	var FormValidator = function () {
		...
		var custom_field_form = function () {
			...
			$('#custom_field_form').validate({
				...
				messages: {
					ministry_id: "Please select ministry",
