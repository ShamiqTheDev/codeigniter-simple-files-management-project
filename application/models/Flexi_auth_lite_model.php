...
class Flexi_auth_lite_model extends CI_Model
{
	public function __construct()
	{
		...
		// User custom join data table(s)
		$this->auth->tbl_custom_join_data = (! empty($database_config['custom_join'])) ? $database_config['custom_join'] : array();
	...
	public function get_users($sql_select = FALSE, $sql_where = FALSE, $sql_group_by = TRUE)
	{
		...
		// Left Join user custom join data table(s) to user account table.
		foreach ($this->auth->tbl_custom_join_data as $table)
		{
			$this->db->join($table['table'], $table['join_table'].'.'.$table['foreign_key'].' = '.$table['table'].'.'.$table['primary_key'], 'left');
		}
