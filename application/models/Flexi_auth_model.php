...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	 ...
	 * @author Rob Hussey
	 */
	public function insert_user($email, $username, $password, $section, $designation, $user_job_group, $custom_data = FALSE, $group_id = FALSE)
	{
	    ...
	    $sql_insert = array(
			...
			$this->auth->tbl_col_user_account['section'] => implode(',', $section),
			$this->auth->tbl_col_user_account['designation'] => $designation,
			$this->auth->tbl_col_user_account['user_job_group'] => $user_job_group
		);
		...
		$this->insert_custom_user_pass_history_data($user_id, $hash_password);
	...
	/**
	 * insert_custom_user_pass_history_data
	 * Inserts data into a custom user table and returns the table name and row id of each record inserted.
	 *
	 * @return array/bool
	 * @author Rob Hussey
	 */
	public function insert_custom_user_pass_history_data($user_id = FALSE, $hash_password = FALSE)
	{
		if (! is_numeric($user_id) || empty($hash_password))
		{
			return FALSE;
		}
		...
		$sql_insert[$this->auth->database_config['pass_history']['columns']['account_id']] = $user_id;
		$sql_insert[$this->auth->database_config['pass_history']['columns']['password']] = $hash_password;

		$this->db->insert($this->auth->database_config['pass_history']['table'], $sql_insert);
		
		// Get new table row id.
		if ($this->db->affected_rows() > 0) 
		{
			$row_data[$this->auth->database_config['pass_history']['table']] = $this->db->insert_id();
		}
	 ...
	 * @author Rob Hussey
	 */
	public function check_last_password($user_id, $identity, $verify_password)
	{
		...
		// check last password	
		$sql_select = array(
			$this->auth->database_config['pass_history']['columns']['password'],
			$this->auth->database_config['pass_history']['columns']['date']
		);
		
		$query = $this->db->select($sql_select)
			->where($this->auth->database_config['pass_history']['columns']['account_id'], $user_id)
			->limit(5)
			->order_by($this->auth->database_config['pass_history']['columns']['date'], 'DESC')
			->get($this->auth->database_config['pass_history']['table']);
		...
		foreach($result as $get_data) {
			...
			$database_password = $get_data[$this->auth->database_config['pass_history']['columns']['password']];
			$static_salt = $this->auth->auth_security['static_salt'];
			
			if($hash_token->CheckPassword($database_salt . $verify_password . $static_salt, $database_password)) { //die('TEST');
				return true;
			}
	...
	public function change_password($identity, $current_password, $new_password, $user_id)
	{
		...
		if($this->check_last_password($user_id, $identity, $new_password)) {
			$this->set_error_message('account_pass_privous', 'config');
			return false;
		}
	    ...
	    if ($this->verify_password($identity, $current_password))
	    {
			...
			$this->insert_custom_user_pass_history_data($user_id, $hash_new_password);
	...
	public function login($identity = FALSE, $password = FALSE, $remember_user = FALSE)
	{
		...
		$sql_select = array(
			...
			$this->auth->tbl_col_user_account['section'],
			$this->auth->tbl_col_user_account['user_job_group']
		);
	    ...
	    {	
			$user = $query->row();
			$this->session->set_userdata(array('section'  => $user->uacc_section_fk));
			$this->session->set_userdata(array('user_job_group'  => $user->uacc_user_job_group_fk));
			...
			// Check if password has been expire.
			$this->session->set_userdata(array('pass_expire_remaining_days' => 0));
			
			if(!in_array($user->uacc_group_fk, $this->auth->auth_security['admin_group_id'])) {
				if($this->check_password_expire($user->uacc_id)) {
					$this->set_error_message('account_pass_expire', 'config');
					return FALSE;
				}
			}
	 ...
	 * Verify that a submitted password has been expire.
	 *
	 * @return bool
	 * @author Rob Hussey
	 */
	public function check_password_expire($user_id)
	{
		...
		$pass_date = date('Y-m-d', strtotime($result->{$this->auth->database_config['pass_history']['columns']['date']}));
		
		$datetime1 = new DateTime($pass_date);
		$datetime2 = new DateTime($current_date);
		$interval = $datetime1->diff($datetime2);
				
		$this->session->set_userdata(array('pass_expire_remaining_days' => $this->auth->auth_security['password_expire_days'] - $interval->days));
		
		if($interval->days > $this->auth->auth_security['password_expire_days']) {
			return true;
		}
