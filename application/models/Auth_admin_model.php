...
class Auth_admin_model extends CI_Model {
	...
	function get_user_accounts()
	{
		...
		$sql_select = array(
			...
			$this->flexi_auth->db_column('user_acc', 'section'),
			'upro_first_name',
			'upro_last_name',
			'GROUP_CONCAT(user_section.`sectionName` ORDER BY user_section.`sectionId`) AS section_name'
		...
		$this->flexi_auth->sql_join($sql_join, $sql_join_on, 'LEFT');
		
		/*if($this->auth->database_config['custom_join']['section']) {
			foreach($this->auth->database_config['custom_join']['section'] as $get_custom_join) {
	...
	function update_user_account($user_id)
	{
		...
		$validation_rules = array(
			...
			//array('field' => 'update_section[]', 'label' => 'Section', 'rules' => 'required'),
		...
		if ($this->form_validation->run())
		{
			...
			$profile_data = array(
				...
				$this->flexi_auth->db_column('user_acc', 'section') => implode(',', $this->input->post('update_section')),
				$this->flexi_auth->db_column('user_acc', 'designation') => $this->input->post('update_designation'),
				$this->flexi_auth->db_column('user_acc', 'group_id') => $this->input->post('update_group'),
				$this->flexi_auth->db_column('user_acc', 'user_job_group') => $this->input->post('update_user_job_group')
			);
