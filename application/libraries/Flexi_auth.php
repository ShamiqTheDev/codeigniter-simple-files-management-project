...
class Flexi_auth extends Flexi_auth_lite
{
	...
	public function valid_password_chars($password = FALSE)
	{
		...
		if((bool) preg_match($this->CI->auth->auth_security['valid_password_special_chars'], $password))
	 ...
	 * @author Rob Hussey
	 */
	public function insert_user($email, $username = FALSE, $password, $section, $designation, $user_job_group, $user_data, $group_id = FALSE, $activate = FALSE) 
	{
		//echo '<pre>'; print_r($section); die();
	
		$user_id = $this->CI->flexi_auth_model->insert_user($email, $username, $password, $section, $designation, $user_job_group, $user_data, $group_id);
	...
	public function update_user($user_id, $user_data)
	{
		if ($this->CI->flexi_auth_model->update_user($user_id, $user_data))
		{
			...
			if($user_id == $this->get_user_id()){
				$this->CI->session->set_flashdata('msg_Flexi_auth_division', 1);
